...
function RegistrationForms({
  ...
  return (
    <div className="card-container">
      <Tabs type="card">
        <TabPane tab={<span>Company info { isEmpty(companyData) ? '' : <Icon type="check" />}</span>} key="1">
          <CompanyInfoForm data={companyData} />
        </TabPane>
        <TabPane tab={<span>Contact details { isEmpty(contactData) ? '' : <Icon type="check" />}</span>} key="2">
          <ContactForm data={contactData} />
        </TabPane>
        <TabPane tab={<span>Management Team { isEmpty(managementData) ? '' : <Icon type="check" />}</span>} key="3">
          <ManagementForm data={managementData} />
        </TabPane>
        <TabPane tab={<span>Company shareholder information { isEmpty(shareholderData) ? '' : <Icon type="check" />}</span>} key="4">
          <ShareholderForm data={shareholderData} />
        </TabPane>
        <TabPane tab={<span>Group information { isEmpty(groupData) ? '' : <Icon type="check" />}</span>} key="5">
          <GroupForm data={groupData} />
        </TabPane>
        <TabPane tab={<span>Products & services { isEmpty(productData) ? '' : <Icon type="check" />}</span>} key="6">
          <ProductsForm data={productData} />
        </TabPane>
        <TabPane tab={<span>Capacity building certificate { isEmpty(certificateData) ? '' : <Icon type="check" />}</span>} key="7">
