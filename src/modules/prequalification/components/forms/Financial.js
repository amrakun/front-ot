...
import { Form, Select, Button, Row, Col, DatePicker, Input,
   Upload, Icon } from 'antd';
import { yearData, booleanData, currencyData, dateFormat } from '../../constants'
...
class PrequalificationForm extends React.Component {
  ...
  renderItem(label, id) {
    ...
    return (
      <FormItem className="multiple-wrapper"
        label={label}
        extra={id === 'turnover' ? 'Turnover may be used by buyers as a search criteria. Currency conversions based on exchange rates on the day of the search will be applied to your answer for buyers searching in a different currency' : ''}
        {...formItemLayout}
      >
        <Row gutter={16}>
          <Col span={12}>
            <FormItem>
              {getFieldDecorator(`${id}Year1`, {
                rules: [{ required: canProvide, message: 'Please select an year!' }]
              })(
          ...
          <Col span={12}>
            <FormItem>
              {getFieldDecorator(`${id}Data1`, {
                rules: [{ required: canProvide, message: 'Please enter an input!' }]
              })(
                <Input />
              )}
            </FormItem>
        ...
        <Col span={12}>
          <FormItem>
            {getFieldDecorator(`recordUpload${i}`, {
              ...
              <Upload name={`recordUpload${i}`} action="/upload.do" listType="picture">
                <Button>
  ...
  render() {
    ...
    const currencyOptions = currencyData.map((el, i) => <Option key={i}>{el}</Option>);
